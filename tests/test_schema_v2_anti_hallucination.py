...
class TestSchemaV2Structure:
    ...
    def test_total_fields_is_39(self):
        """Schéma V2 doit avoir exactement 39 champs"""
        assert len(FIELD_SPECS_V2) == 39, (
            f"Expected 39 fields, got {len(FIELD_SPECS_V2)}"
        )
    
    def test_field_types_distribution(self):
        """Distribution: 5 deterministic, 13 narrative, 10 list, 11 enum"""
        by_type = {}
        for spec in FIELD_SPECS_V2.values():
            by_type[spec.field_type] = by_type.get(spec.field_type, 0) + 1
        
        assert by_type["deterministic"] == 5
        assert by_type["narrative"] == 13
        assert by_type["list"] == 10
        assert by_type["enum"] == 11
    
    def test_narrative_max_chars_3000(self):
        """Tous les champs narrative doivent avoir max_chars=3000"""
        ...
        for key in narrative_fields:
            ...
            assert spec.max_chars == 3000, (
                f"{key}: Expected max_chars=3000, got {spec.max_chars}"
            )
...
"""
    ...
    def test_extract_cecrl_never_invents(self):
        ...
        # Ces textes ne doivent PAS retourner de niveau
...
class TestTitleMapping:
    ...
    def test_all_34_fields_mappable(self):
        """Tous les 34 champs (sauf deterministic) doivent être mappables"""
        # 39 total - 5 deterministic = 34 mappables
        from core.title_mapping_v2 import get_mapping_stats
        
        stats = get_mapping_stats()
        assert stats["unique_fields_mapped"] == 34, (
            f"Expected 34 mappable fields, got {stats['unique_fields_mapped']}"
        )
...
class TestAntiHallucinationValidation:
    ...
    def test_list_should_limit_to_4_items(self):
        ...
        # Test conceptuel: si LLM retourne 6 items, tronquer à 4
    ...
    def test_require_sources_fields(self):
        """Champs avec require_sources=True"""
        required_fields = [
            "CV",
        ...
        # Tous les champs enum
        enum_fields = list_fields_by_type("enum")
        required_fields.extend(enum_fields)
...
class TestFormationVsFormationsTests:
    """Test collision FORMATION vs FORMATIONS_TESTS"""
    ...
    def test_formations_tests_is_list(self):
        """FORMATIONS_TESTS doit être list"""
        spec = get_field_spec_v2("FORMATIONS_TESTS")
        assert spec.field_type == "list"
    
    def test_no_collision_in_specs(self):
        """Pas de collision entre FORMATION et FORMATIONS_TESTS"""
        assert "FORMATION" in FIELD_SPECS_V2
        assert "FORMATIONS_TESTS" in FIELD_SPECS_V2
        assert "FORMATION" != "FORMATIONS_TESTS"
